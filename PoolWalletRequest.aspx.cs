...
partial class PoolWalletRequest : System.Web.UI.Page
{
    ...
    protected void SaveRequest()
    {
        try
        {
            ...
            if (updateeffect > 0)
            {
                ...
                try
                {
                    if (flag == true)
                    {
                        ...
                        string str = "INSERT INTO PoolWalletReq(ReqNo,ReqDate,Formno,PID,Paymode,Amount,ChqNo,ChqDate,BankName,BranchName," +
                            ...
                            "'" + DDlBank.SelectedValue + "','" + TxtIssueBranch.Text + "','" + Txtwalletad.Text + "' FROM PoolWalletReq " + "; " +
                        ...
                        int ReqNo;
                        str = " Select Max(ReqNo) as ReqNo FROM PoolWalletReq WHERE Formno='" + Session["Formno"] + "' AND Amount='" + TxtAmount.Text + "'";
                        dt1 = new DataTable();
                        dt1 = obj.GetData(str);
                        if (dt1.Rows.Count > 0)
                        {
            ...
            else
            {
                ...
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "location.replace('PoolWalletRequest.aspx');", true);
                return;
            }
        }
        catch (Exception ex)
    ...
    protected void TxtDDNo_TextChanged(object sender, System.EventArgs e)
    {
        try
        {
            ...
            if (Convert.ToInt32(DdlPaymode.SelectedValue) != 8)
            {
                ...
                {
                    s = "select * from PoolWalletReq where ChqNo='" + (TxtDDNo.Text) + "' and IsApprove<>'R'";
                    dt = obj.GetData(s);
                    if (dt.Rows.Count > 0)
                    {
