...
public partial class DigitalWallet : System.Web.UI.Page
{
    ...
    private void FillBalance()
    {
        try
        {
            query = ObjDal.Isostart + "Select * From dbo.ufnGetBalance('" + Session["FormNo"] + "','D')" + ObjDal.IsoEnd;
    ...
    private void FillDetail()
    {
        ...
        {
            DataTable Dt=new DataTable ();
            query = ObjDal.Isostart + "Select Row_Number() Over(Order by VoucherId Desc) As Rn,* From [V#RptDigitalwallet] Where FormNo='" + Session["Formno"] + "'" + ObjDal.IsoEnd;
            Dt = SqlHelper.ExecuteDataset (constr1 ,CommandType.Text, query).Tables[0];
            {
