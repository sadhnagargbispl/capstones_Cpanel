...
public partial class FranchiseLevelIncome : System.Web.UI.Page
{
    ...
    private void Fill_Grid()
    {
        try
        {
            ...
            DataSet Ds = new DataSet();
             str = Objdal.Isostart + " exec sp_GetFranchiseLevelIncome '" + Session["FormNo"] + "' " + Objdal.IsoEnd;
            dt = SqlHelper.ExecuteDataset(constr1, CommandType.Text, str).Tables[0];
            Session["FranchiselevelIncome"] = dt;
            if (dt.Rows.Count > 0)
            {
    ...
    protected void btnRefresh_Click(object sender, EventArgs e)
    {
        try
        {
            Session["FranchiselevelIncome"] = null;
            Fill_Grid();
        }
        catch (Exception ex)
        {
